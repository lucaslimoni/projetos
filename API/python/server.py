...
class Users(Resource):
    ...
    def post(self):
        ...
        query_email = conn.execute("select * from user where email ='" + str(email) + "'")
        resp_email = [dict(zip(tuple(query_email.keys()), i)) for i in query_email.cursor]
        if(len(resp_email) == 0):
            ...
            return jsonify("Email ja cadastrado")
